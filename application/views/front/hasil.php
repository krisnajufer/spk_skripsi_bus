<?php
$id_perusahaan = $hasil['id_perusahaan'];
$result = array();
for ($i=0; $i < sizeof($hasil['perhitungan']); $i++) { 
	$result[$i] = [
		'id_perusahaan' => $id_perusahaan[$i],
		'final_score' => $hasil['perhitungan'][$id_perusahaan[$i]]['final_score'][0]
	];
}
...
for ($j=0; $j < sizeof($result); $j++) { 
	for ($k=0; $k < sizeof($hasil['perhitungan'][$result[$j]['id_perusahaan']]['value_utilities']); $k++) { 
		$temp_hasil[$j][$k] = [
			'alternatif' => select_smart($result[$j]['id_perusahaan']),
			'kriteria' => select_kriteria($hasil['id_kriteria'][$k]),
			'normalisasi' => $hasil['perhitungan'][$result[$j]['id_perusahaan']]['normalisasi'][$k],
			'values_util' => $hasil['perhitungan'][$result[$j]['id_perusahaan']]['value_utilities'][$k],
			'total' => $hasil['perhitungan'][$result[$j]['id_perusahaan']]['total'][$k],
			'skor' => $hasil['perhitungan'][$result[$j]['id_perusahaan']]['final_score'][0]
		];
	}
}
?>
	...
	<div class="col-md-12">
		<div class="card shadow animated fadeInDownBig border-bottom-info shadow h-100 py-2 my-3" id="manual">
			...
			<div class="card-body">
				...
				<?php $value = select_smart($result[$j]['id_perusahaan']) ?>
					<div class="<?= $cardsize ?> <?= $size ?> mb-2">
						<div class="card border-0 shadow-sm h-100">
							...
							<div class="card-body">
								<center><img class="img-responsive img-fluid mb-2" src="<?= base_url('assets/img/perusahaan/'.$value->foto) ?>" <?= $imgsize ?>></center>
								<ul class="px-3 my-0">
									<li><?= $value->namap.' '.$value->bentuk ?></li>
									<li><?= $value->toilet.' - '.$value->smoking ?></li>
			...
			<div class="collapse" id="cardRincian">
				<div class="card-body">
					<?php for ($k=0; $k < $limit; $k++) { ?>
						<?php $value = select_smart($result[$k]['id_perusahaan']) ?>
						<div class="row">
							<div class="col-md-10">
								<h5>Rangking : <?= $k+1 ?></h5>
								<h4 class="font-weight-bold text-primary"><?= ' '.$value->namap.' '. $value->bentuk ?></h4>
							</div>
							<div class="col-md-2">
							</div>
						</div>
